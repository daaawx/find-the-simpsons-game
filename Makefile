EXECUTABLE		:= a.out			
	...
	#What are our source files, and where are they?  All .c, .cpp, and .cc files
C_SOURCES 		= $(wildcard *.c)
CPP_SOURCES		= $(wildcard *.cpp)
CC_SOURCES 		= $(wildcard *.cc)

	#So what are our object files, and where are they?  Anything we can, with those source files
...
INCLUDE_FLAGS	= `sdl2-config --cflags` -Ilib/SSDL/include

											#Tell g++ where to find library files for SDL2, SSDL
LIBRARIES		= ssdl SDL2main SDL2 SDL2_image SDL2_ttf SDL2_mixer
LIBRARY_DIRS	= lib/SSDL/unix
